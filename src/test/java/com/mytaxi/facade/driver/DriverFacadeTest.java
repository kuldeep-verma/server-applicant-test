...
public class DriverFacadeTest
{
    ...
    public void testFindDriverByUsername() throws EntityNotFoundException
    {
        DriverDO mockOutputDriverDO = new DriverDO("TestUser", "pass");
        mockOutputDriverDO.setId(22l);
        when(driverService.findDriverByUsername(any(String.class))).thenReturn(mockOutputDriverDO);

        DriverDO result = defaultDriverFacade.findDriverByUsername("TestUser");
        assertEquals(mockOutputDriverDO, result);
        verify(driverService, times(1)).findDriverByUsername(any(String.class));
    }


    @Test(expected = EntityNotFoundException.class)
    public void testFindDriverByUsernameThrowExceptionWhenUserNotFound() throws EntityNotFoundException
    {
        when(driverService.findDriverByUsername(any(String.class))).thenThrow(new EntityNotFoundException("User not found"));

        defaultDriverFacade.findDriverByUsername("username");
    ...
    public void testFindDriversByCarAttributes() throws EntityNotFoundException
    {
        ...
        when(driverService.findDriversByCarAttributes(any(CarDO.class))).thenReturn(driverDOs);

        List<DriverDO> resultDriverDOs = defaultDriverFacade.findDriversByCarAttributes(carDO);
        assertEquals(driverDOs, resultDriverDOs);
    }


    @Test(expected = EntityNotFoundException.class)
    public void testFindDriversByCarAttributesThrowExceptionWhenNoResult() throws EntityNotFoundException
    {
        CarDO carDO = new CarDO(132l, ZonedDateTime.now(), "White", "PK 101", "Gas", 5, true, false, null);

        when(driverService.findDriversByCarAttributes(any(CarDO.class))).thenThrow(new EntityNotFoundException("No result found"));

        defaultDriverFacade.findDriversByCarAttributes(carDO);
        verify(driverService, times(1)).findDriverByUsername(any(String.class));
    }
